...
- DevQL Test Harness pack: `builtin_packs(repo_root)` opens an optional relational handle shared by all test-harness ingesters; linkage / classification / summaries ingesters use the same pattern. `bitloops testlens ingest-tests` / `ingest-coverage` / `ingest-coverage-batch` invoke the host ingesters.
