...
The `bitloops-local-embeddings` and `bitloops-platform-embeddings` binaries are released separately in `bitloops/bitloops-embeddings`. `bitloops init --install-default-daemon` now offers Bitloops cloud, local runtime, or skip during interactive onboarding; `bitloops enable --install-embeddings` and `bitloops embeddings install` still default to the managed local runtime unless you pass the platform flags. If you are building from source or using a custom runtime, install the matching binary separately.
