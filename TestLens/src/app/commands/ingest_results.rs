...
pub fn handle(db_path: &Path, jest_json_path: &Path, commit_sha: &str) -> Result<()> {
    ...
    let mut stats = IngestResultsStats::default();
    ...
    for suite in &jest.test_results {
        ...
        for assertion in &suite.assertion_results {
            ...
            let Some(test_artefact_id) = scenario_map.get(&key) else {
                eprintln!(
            ...
            let run_id = format!("run:{commit_sha}:{test_artefact_id}");
    ...
    println!(
        ...
        commit_sha, stats.ingested, stats.unmatched
    );
    Ok(())
}

fn build_scenario_map(
    scenarios: Vec<crate::domain::TestScenarioRecord>,
    ...
    for scenario in scenarios {
        ...
        map.insert(key, scenario.artefact_id);
    }
    map
}
