...
use crate::app::test_mapping::file_discovery::{
    line_count, looks_like_inline_rust_test_source, read_source_file,
};
...
impl LanguageProvider for RustLanguageProvider {
    ...
    fn reconcile(
        ...
        let enumerated_scenarios = enumeration
            ...
            .into_iter()
            .filter(|scenario| {
                let normalized_key = if scenario.discovery_source == ScenarioDiscoverySource::Doctest
                {
                    scenario
                        .reference_candidates
                        .iter()
        ...
        ReconciledDiscovery { enumerated_scenarios }
    }
}
