...
use super::rstest::{applied_template_name, build_rust_rstest_cases, collect_rust_rstest_templates};
            ...
            && let Some(body) = super::macros::extract_rust_macro_invocation_body(raw_invocation)
        {
                ...
                .collect::<Vec<_>>()
            {
                let simple = symbol
                    ...
                    .trim();
                if !simple.is_empty()
                    && simple.chars().next().is_some_and(|ch| ch.is_ascii_alphabetic() || ch == '_')
