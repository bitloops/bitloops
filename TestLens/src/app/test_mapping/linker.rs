...
pub(crate) fn doctest_match_keys(
    ...
    for candidate in reference_candidates {
        if let ReferenceCandidate::ExplicitTarget { path, start_line } = candidate {
            keys.insert(normalized_enumerated_doctest_key(path, &item_name, *start_line));
        }
    }
    if keys.is_empty() {
...
pub(crate) fn normalized_enumerated_doctest_key(path: &str, item_name: &str, start_line: i64) -> String {
