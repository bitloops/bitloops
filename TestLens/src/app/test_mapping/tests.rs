...
"#;
    ...
    assert!(
        async_scenario.reference_candidates.contains(&ReferenceCandidate::SymbolName(
            "run".to_string()
        )),
...
"#;
    ...
    assert!(
        suites[0].scenarios[0]
            .reference_candidates
            .contains(&ReferenceCandidate::SymbolName("render_message".to_string())),
...
fn rust_test_context_paths_include_parent_module_for_property_test_files() {
    let context_paths =
        rust_test_context_source_paths("crates/red_knot_python_semantic/src/types/property_tests.rs");
...
"#;
    ...
    assert_eq!(inner.scenarios.len(), 1, "expected exactly one inner scenario");
