...
fn print_json<T: Serialize>(value: &T) -> Result<()> {
    let json = serde_json::to_string_pretty(value).context("failed serializing query JSON")?;
    println!("{json}");
    Ok(())
}
...
mod tests {
    ...
    fn verification_level_is_partial_without_coverage_or_below_threshold() {
        ...
        assert_eq!(derive_verification_level(2, Some(&details)), "partially_tested");
    }
    ...
    fn strength_uses_classification_weights() {
        ...
        assert_eq!(compute_strength(1, "integration"), INTEGRATION_STRENGTH_WEIGHT);
        assert_eq!(compute_strength(1, "e2e"), E2E_STRENGTH_WEIGHT);
        assert_eq!(
            compute_strength(1, "unknown"),
