...
export default class Options {
	...
	private static isNumeric(str: string): boolean {
		...
		return !isNaN(+str) && // use type coercion to parse the string (`parseFloat` alone does not do this)
			!isNaN(parseFloat(str)); // for whitespace strings to fail
	}
	...
	static getVersion(): string {
		const version = process.env[MQTopics.VERSION];
		this.options[MQTopics.VERSION] = replaceAllCharacters(version, '.', '')
		return this.options[MQTopics.VERSION];
	}
