...
- `@sync_known_gap` marks producer scenarios that describe the target contract but currently expose product gaps; the producer alias excludes them until the product fix lands. Current known gaps cover hook/watcher races around post-commit and post-merge, plus `git reset --hard` and `git clean -fd` drift after indexed uncommitted files are removed.
