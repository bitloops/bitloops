...
use super::taxonomy::{
    ...
    AssignmentSource, AssignmentStatus, RoleSignalPolarity, RoleTarget, TargetKind, assignment_id,
};
...
struct UnknownTargetPolicyOutcome {
    adjudication_requests: Vec<RoleAdjudicationRequest>,
    rule_mining_inputs: Vec<super::rule_mining::RoleRuleMiningClusterInput>,
    unknown_targets_total: usize,
    suppressed_non_role: usize,
    rule_mining_eligible: usize,
    adjudication_escalated: usize,
}
...
fn path_suppressed_for_roles(path: &str) -> bool {
...
fn select_unknown_target_policy(
    repo_id: &str,
    generation_seq: u64,
    target_summaries: &BTreeMap<RoleTarget, RoleTargetSummary>,
    ...
    for summary in target_summaries.values() {
        ...
        match unknown_target_disposition(summary, &file_targets_by_path) {
            ...
            UnknownTargetDisposition::RuleMiningCandidate => {
                ...
                outcome
                    .rule_mining_inputs
                    .push(super::rule_mining::RoleRuleMiningClusterInput {
