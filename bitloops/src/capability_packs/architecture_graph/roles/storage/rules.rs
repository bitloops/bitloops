...
pub async fn upsert_detection_rule(
    ...
    let sql = format!(
        "INSERT INTO architecture_role_detection_rules (
            ...
            score_json, provenance_json, updated_at
         ) VALUES (
            {repo_id}, {rule_id}, {role_id}, {version}, {lifecycle},
            {candidate_selector}, {positive_conditions}, {negative_conditions},
            {score_json}, {provenance}, {now}
         )
...
pub async fn load_active_detection_rules(
    ...
    let sql = format!(
        "SELECT repo_id,
         ...
         WHERE rule.repo_id = {} AND rule.lifecycle_status = 'active'
           AND rule.version = (
