...
pub(crate) async fn determine_repo_embedding_sync_action(
    ...
    let current_coverage_complete =
        current_repo_semantic_clone_rows_are_complete(relational, repo_id, setup).await?;
    ...
    Ok(
        if current_coverage_complete && current_setups.len() == 1 && current_setups[0] == *setup {
...
mod semantic_embedding_persistence_tests {
    ...
    async fn sqlite_relational_with_embedding_state_schema() -> RelationalStorage {
        sqlite_relational_with_schema(&format!(
            "{}\nCREATE TABLE symbol_semantics (artefact_id TEXT PRIMARY KEY, summary TEXT);\nCREATE TABLE symbol_features (artefact_id TEXT PRIMARY KEY);\nCREATE TABLE artefacts_current (repo_id TEXT NOT NULL, artefact_id TEXT PRIMARY KEY, path TEXT, start_line INTEGER, symbol_id TEXT, canonical_kind TEXT, language_kind TEXT);",
    ...
    async fn insert_fully_indexed_current_artefact(
        ...
        relational
            ...
            .await
            .expect("insert features row");
        relational
            .exec(&format!(
                "INSERT INTO symbol_embeddings (artefact_id, repo_id, blob_sha, provider, model, dimension, embedding_input_hash, embedding)
                 VALUES ('{artefact_id}', 'repo-1', 'blob-1', '{provider}', '{model}', {dimension}, 'hash-{artefact_id}', '[0.1,0.2,0.3]')",
                artefact_id = esc_pg(artefact_id),
