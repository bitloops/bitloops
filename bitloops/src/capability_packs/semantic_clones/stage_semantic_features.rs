...
use self::storage::{
    build_conditional_current_semantic_persist_rows_sql, build_current_repo_artefacts_sql,
...
async fn persist_current_semantic_feature_rows_for_matching_input(
    relational: &RelationalStorage,
    input: &semantic::SemanticFeatureInput,
    rows: &semantic::SemanticFeatureRows,
) -> Result<()> {
...
mod tests {
    ...
    use super::{
        ...
        ensure_required_llm_summary_output,
        persist_current_semantic_feature_rows_for_matching_input,
        remap_semantic_input_to_current_artefact, semantic_features_sqlite_schema_sql,
