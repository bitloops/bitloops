...
use crate::cli::inference::{
    ContextGuidanceSetupOutcome, ContextGuidanceSetupSelection, InferenceCommand,
...
#[test]
fn inference_install_rejects_architecture_flags() {
    ...
    let Err(err) = result else {
        panic!("architecture-specific inference install flags should be removed");
    };
...
#[test]
fn inference_install_without_architecture_flags_still_parses() {
    let cli = crate::cli::Cli::try_parse_from(["bitloops", "inference", "install"])
        .expect("parse base inference install");

    let crate::cli::Commands::Inference(args) = cli.command.expect("inference command") else {
        panic!("expected inference command");
    };
    assert!(matches!(args.command, Some(InferenceCommand::Install)));
}
