...
async fn execute_embedding_job(
    ...
    if sync_action == RepoEmbeddingSyncAction::AdoptExisting {
        if let Err(err) = persist_active_embedding_setup(relational, &job.repo_id, &setup).await {
...
"#,
    ...
    fn load_current_embedding_setups(
        sqlite_path: &Path,
        repo_id: &str,
    ...
    async fn seed_current_state_and_semantics(
        ...
        execute_ingest_with_observer(&cfg, true, 10, None, None)
            .await
        ...
        let inputs = crate::capability_packs::semantic_clones::load_semantic_feature_inputs_for_current_repo(
            &relational,
            repo_root,
            &cfg.repo.repo_id,
        )
    ...
    async fn daemon_embedding_job_bootstraps_active_setup_from_single_runtime() {
        ...
        let outcome =
            run_embedding_job_with_env(&job, "local_fastembed", "bootstrap-model", "3").await;
    ...
    async fn daemon_embedding_job_treats_dimension_change_as_setup_change() {
        ...
        let first =
            run_embedding_job_with_env(&job, "local_fastembed", "dimension-model", "3").await;
        assert!(first.error.is_none());
        let first_rows = load_current_embedding_rows(&sqlite_path, &cfg.repo.repo_id);
        let first_hashes = hash_by_symbol(&first_rows);

        let second =
            run_embedding_job_with_env(&job, "local_fastembed", "dimension-model", "4").await;
        let second_rows = load_current_embedding_rows(&sqlite_path, &cfg.repo.repo_id);
        let second_hashes = hash_by_symbol(&second_rows);
    ...
    async fn daemon_embedding_job_keeps_incremental_behavior_when_setup_is_unchanged() {
        ...
        let first =
            run_embedding_job_with_env(&full_job, "local_fastembed", "stable-model", "3").await;
        assert!(first.error.is_none());
        ...
        let second =
            run_embedding_job_with_env(&incremental_job, "local_fastembed", "stable-model", "3")
                .await;
