...
const ATTACHED_RUNTIME_STORE_PREFIX: &str = "runtime_store.";
const DIRECT_RUNTIME_STORE_PREFIX: &str = "";

pub(super) fn open_semantic_writer_connection(
    runtime_db_path: &Path,
    relational_db_path: &Path,
) -> Result<Connection> {
    ensure_runtime_db_exists(runtime_db_path)?;
    ensure_relational_db_exists(relational_db_path)?;
    ...
    configure_semantic_writer_connection(&conn, "semantic writer connection")?;
    conn.execute(
...
pub(super) fn open_semantic_writer_relational_connection(
    relational_db_path: &Path,
) -> Result<Connection> {
    ensure_relational_db_exists(relational_db_path)?;
    crate::sqlite_vec_auto_extension::register_sqlite_vec_auto_extension()
        .context("registering sqlite-vec auto-extension for semantic embedding connection")?;
    let conn = Connection::open_with_flags(relational_db_path, OpenFlags::SQLITE_OPEN_READ_WRITE)
        .with_context(|| {
        format!(
            "opening SQLite database at {}",
    ...
    configure_semantic_writer_connection(&conn, "semantic embedding relational connection")?;
    Ok(conn)
}

pub(super) fn open_semantic_writer_runtime_connection(
    runtime_db_path: &Path,
) -> Result<Connection> {
    ensure_runtime_db_exists(runtime_db_path)?;
    let conn = Connection::open_with_flags(runtime_db_path, OpenFlags::SQLITE_OPEN_READ_WRITE)
        .with_context(|| format!("opening SQLite database at {}", runtime_db_path.display()))?;
    configure_semantic_writer_connection(&conn, "semantic embedding runtime connection")?;
    Ok(conn)
}
...
pub(super) fn upsert_runtime_embedding_mailbox_item(
    ...
    upsert_runtime_embedding_mailbox_item_in_store(tx, ATTACHED_RUNTIME_STORE_PREFIX, repo, item)
}

pub(super) fn upsert_runtime_embedding_mailbox_item_direct(
    tx: &Transaction<'_>,
    repo: &SemanticBatchRepoContext,
    item: &SemanticEmbeddingMailboxItemInsert,
) -> Result<()> {
    upsert_runtime_embedding_mailbox_item_in_store(tx, DIRECT_RUNTIME_STORE_PREFIX, repo, item)
}

fn upsert_runtime_embedding_mailbox_item_in_store(
    tx: &Transaction<'_>,
    runtime_store_prefix: &str,
    repo: &SemanticBatchRepoContext,
    item: &SemanticEmbeddingMailboxItemInsert,
) -> Result<()> {
    let existing = tx
        .query_row(
            &format!(
                "SELECT item_id, status
                 FROM {runtime_store_prefix}semantic_embedding_mailbox_items
                 WHERE repo_id = ?1
    ...
    if let Some((item_id, status)) = existing {
        if status == "pending" {
            tx.execute(
                &format!(
                    "UPDATE {runtime_store_prefix}semantic_embedding_mailbox_items
    ...
    insert_runtime_embedding_mailbox_item_in_store(tx, runtime_store_prefix, repo, item)
}

fn insert_runtime_embedding_mailbox_item_in_store(
    tx: &Transaction<'_>,
    runtime_store_prefix: &str,
    repo: &SemanticBatchRepoContext,
    item: &SemanticEmbeddingMailboxItemInsert,
    ...
    tx.execute(
        &format!(
            "INSERT INTO {runtime_store_prefix}semantic_embedding_mailbox_items (
...
pub(super) fn delete_runtime_embedding_mailbox_items_direct(
    tx: &Transaction<'_>,
    lease_token: &str,
    item_ids: &[String],
) -> Result<()> {
    delete_runtime_embedding_mailbox_items_in_store(
        tx,
        DIRECT_RUNTIME_STORE_PREFIX,
        lease_token,
        item_ids,
    )
}

fn delete_runtime_embedding_mailbox_items_in_store(
    tx: &Transaction<'_>,
    runtime_store_prefix: &str,
    lease_token: &str,
    item_ids: &[String],
    ...
    tx.execute_batch(&format!(
        "DELETE FROM {runtime_store_prefix}semantic_embedding_mailbox_items
         WHERE lease_token = '{lease_token}'
           AND item_id IN ({item_ids})",
        runtime_store_prefix = runtime_store_prefix,
        lease_token = esc_pg(lease_token),
        item_ids = sql_string_list(item_ids),
    ))
...
pub(super) fn upsert_runtime_clone_rebuild_signal_direct(
    tx: &Transaction<'_>,
    repo: &SemanticBatchRepoContext,
    signal: &CapabilityWorkplaneJobInsert,
) -> Result<()> {
    upsert_runtime_clone_rebuild_signal_in_store(tx, DIRECT_RUNTIME_STORE_PREFIX, repo, signal)
}

fn upsert_runtime_clone_rebuild_signal_in_store(
    tx: &Transaction<'_>,
    runtime_store_prefix: &str,
    repo: &SemanticBatchRepoContext,
    signal: &CapabilityWorkplaneJobInsert,
        ...
        .query_row(
            &format!(
                "SELECT job_id, status
                 FROM {runtime_store_prefix}capability_workplane_jobs
                 WHERE repo_id = ?1
                   AND capability_id = ?2
                   AND mailbox_name = ?3
                   AND dedupe_key = ?4
    ...
    if let Some((job_id, status)) = existing {
        if status == "pending" {
            tx.execute(
                &format!(
                    "UPDATE {runtime_store_prefix}capability_workplane_jobs
    ...
    tx.execute(
        &format!(
            "INSERT INTO {runtime_store_prefix}capability_workplane_jobs (
...
fn ensure_runtime_db_exists(runtime_db_path: &Path) -> Result<()> {
...
fn ensure_relational_db_exists(relational_db_path: &Path) -> Result<()> {
...
fn configure_semantic_writer_connection(conn: &Connection, context: &str) -> Result<()> {
    conn.busy_timeout(Duration::from_secs(30))
        .with_context(|| format!("setting {context} busy timeout"))?;
    conn.execute_batch("PRAGMA foreign_keys = ON; PRAGMA synchronous = NORMAL;")
        .with_context(|| format!("configuring {context}"))?;
    Ok(())
}
