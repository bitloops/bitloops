...
fn test_hard_reset_with_protection_preserves_protected_dirs() {
    ...
    fs::create_dir_all(dir.path().join(".bitloops/checkpoint-artifacts"))
        .expect("create .bitloops");
    fs::write(
        dir.path()
            .join(".bitloops/checkpoint-artifacts/session.json"),
    ...
    with_repo_cwd(dir.path(), |_| {
        ...
        let bitloops_content = fs::read_to_string(
            dir.path()
                .join(".bitloops/checkpoint-artifacts/session.json"),
        )
