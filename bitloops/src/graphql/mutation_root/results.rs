...
impl From<crate::host::devql::IngestionCounters> for IngestResult {
    fn from(value: crate::host::devql::IngestionCounters) -> Self {
        Self {
            ...
            file_deltas_upserted: to_graphql_count(value.file_deltas_upserted),
            hunks_upserted: to_graphql_count(value.hunks_upserted),
            added_lines_ingested: to_graphql_count(value.added_lines_ingested),
            deleted_lines_ingested: to_graphql_count(value.deleted_lines_ingested),
            binary_deltas_upserted: to_graphql_count(value.binary_deltas_upserted),
