...
impl RelationalGateway for SqliteRelationalGateway {
    fn initialise_schema(&self) -> Result<()> {
        SqliteRelationalGateway::initialise_schema(self)
    }
    ...
    fn persist_ingestion(
        ...
        SqliteRelationalGateway::persist_ingestion(self, source, item)
    }

    fn insert_relation_assertion(&self, relation: &KnowledgeRelationAssertionRow) -> Result<()> {
        SqliteRelationalGateway::insert_relation_assertion(self, relation)
    }

    fn find_item(&self, repo_id: &str, source_id: &str) -> Result<Option<KnowledgeItemRow>> {
        SqliteRelationalGateway::find_item(self, repo_id, source_id)
    }
    ...
    fn find_item_by_id(
        ...
        SqliteRelationalGateway::find_item_by_id(self, repo_id, knowledge_item_id)
    }

    fn find_source_by_id(&self, knowledge_source_id: &str) -> Result<Option<KnowledgeSourceRow>> {
        SqliteRelationalGateway::find_source_by_id(self, knowledge_source_id)
    }

    fn list_items_for_repo(&self, repo_id: &str, limit: usize) -> Result<Vec<KnowledgeItemRow>> {
        SqliteRelationalGateway::list_items_for_repo(self, repo_id, limit)
    }
...
mod tests {
    ...
    fn sqlite_relational_store_roundtrip_persists_and_finds_item() {
        ...
        let store = SqliteRelationalGateway::new(pool);
