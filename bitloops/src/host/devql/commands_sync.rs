...
async fn execute_sync_inner(
    ...
    let summary_provider: Arc<dyn semantic::SemanticSummaryProvider> = if semantic_clones
        .summary_mode
        == SemanticSummaryMode::Off
    {
        ...
        match semantic_clones_pack::build_semantic_summary_provider(&semantic_provider_config(cfg))
        {
            ...
            Err(err) => {
                log::warn!(
                    "semantic_clones current summaries degraded; using deterministic summaries only: {err:#}"
                );
                Arc::new(semantic::NoopSemanticSummaryProvider)
            }
        }
    ...
    let embedding_outputs_enabled = semantic_clones.embedding_mode
        != SemanticCloneEmbeddingMode::Off
        && embedding_config.embedding_profile.is_some();
    let mut embedding_warning = None;
    let embedding_provider = if embedding_outputs_enabled {
        ...
        .filter(|path| matches!(path.action, sync::types::PathAction::Removed))
    {
        ...
        sync::semantic_projector::remove_path(cfg, relational, &path.path)
            .await
            .with_context(|| {
                format!(
                    "removing current semantic projection for stale path `{}` during DevQL sync",
                    path.path
                )
            })?;
    }
    ...
    for path in classified.iter().filter(|path| {
        ...
        staged_materializations.push((desired, extraction, content));
    }

    // Phase 6: materialize added and changed paths into current-state tables.
    let mut should_refresh_current_clones = counters.paths_removed > 0;
    for (desired, extraction, content) in staged_materializations {
        ...
        .with_context(|| {
            format!(
                "projecting current semantic rows for `{}` during DevQL sync",
                desired.path
            )
        })?;
        should_refresh_current_clones = true;
    }

    if embedding_outputs_enabled && embedding_provider.is_some() {
        if should_refresh_current_clones {
            crate::capability_packs::semantic_clones::pipeline::rebuild_current_symbol_clone_edges(
                relational,
                &cfg.repo.repo_id,
            )
            .await
            .context("rebuilding current semantic clone edges during DevQL sync")?;
        }
    } else {
        ...
        .await
        .context("clearing current semantic clone edges without embeddings during DevQL sync")?;
    }

    if let Some(warning) = embedding_warning.as_deref() {
        log::warn!(
            "semantic_clones current embeddings degraded; skipping current clone edge projection: {warning}"
        );
    }
