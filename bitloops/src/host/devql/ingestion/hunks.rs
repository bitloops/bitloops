...
pub(crate) async fn persist_commit_hunks(
    ...
    repo_id: &str,
    parsed: &ParsedCommitHunks,
) -> Result<()> {
    if parsed.file_deltas.is_empty() && parsed.hunks.is_empty() {
        return Ok(());
    }

    let mut sql_batch = Vec::with_capacity(parsed.file_deltas.len() + parsed.hunks.len());
    for delta in &parsed.file_deltas {
        sql_batch.push(build_upsert_commit_file_delta_sql(
            relational, repo_id, delta,
        ));
    }
    for hunk in &parsed.hunks {
        sql_batch.push(build_upsert_commit_hunk_sql(relational, repo_id, hunk)?);
    }
...
fn build_upsert_commit_file_delta_sql(
    ...
    format!(
        "INSERT INTO commit_file_deltas (
            ...
            old_blob_sha, new_blob_sha, is_binary, updated_at
         ) VALUES (
            '{repo_id}', '{commit_sha}', '{delta_id}', {path_before}, {path_after}, '{change_kind}',
            {old_blob_sha}, {new_blob_sha}, {is_binary}, {now_sql}
         )
         ON CONFLICT (repo_id, delta_id) DO UPDATE SET
            commit_sha = EXCLUDED.commit_sha,
            path_before = EXCLUDED.path_before,
            path_after = EXCLUDED.path_after,
            change_kind = EXCLUDED.change_kind,
            old_blob_sha = EXCLUDED.old_blob_sha,
            new_blob_sha = EXCLUDED.new_blob_sha,
            is_binary = EXCLUDED.is_binary,
            updated_at = {now_sql}",
        repo_id = esc_pg(repo_id),
        commit_sha = esc_pg(&delta.commit_sha),
        delta_id = esc_pg(&delta.delta_id),
        path_before = sql_nullable_text(delta.path_before.as_deref()),
        path_after = sql_nullable_text(delta.path_after.as_deref()),
        change_kind = esc_pg(&delta.change_kind),
        old_blob_sha = sql_nullable_text(delta.old_blob_sha.as_deref()),
        new_blob_sha = sql_nullable_text(delta.new_blob_sha.as_deref()),
        is_binary = bool_sql(relational, delta.is_binary),
        now_sql = now_sql,
    )
}
...
fn build_upsert_commit_hunk_sql(
    ...
    let added_json = serde_json::to_value(&hunk.added_lines)?;
    let deleted_json = serde_json::to_value(&hunk.deleted_lines)?;
    Ok(format!(
        "INSERT INTO commit_hunks (
            ...
            deleted_lines_json, patch, updated_at
         ) VALUES (
            '{repo_id}', '{commit_sha}', '{hunk_id}', '{delta_id}', {path_before}, {path_after}, {hunk_index},
            {old_start}, {old_line_count}, {new_start}, {new_line_count}, {added_lines_json},
            {deleted_lines_json}, '{patch}', {now_sql}
         )
         ON CONFLICT (repo_id, hunk_id) DO UPDATE SET
            commit_sha = EXCLUDED.commit_sha,
            delta_id = EXCLUDED.delta_id,
            path_before = EXCLUDED.path_before,
            path_after = EXCLUDED.path_after,
            hunk_index = EXCLUDED.hunk_index,
            old_start = EXCLUDED.old_start,
            old_line_count = EXCLUDED.old_line_count,
            new_start = EXCLUDED.new_start,
            new_line_count = EXCLUDED.new_line_count,
            added_lines_json = EXCLUDED.added_lines_json,
            deleted_lines_json = EXCLUDED.deleted_lines_json,
            patch = EXCLUDED.patch,
            updated_at = {now_sql}",
        repo_id = esc_pg(repo_id),
        commit_sha = esc_pg(&hunk.commit_sha),
        hunk_id = esc_pg(&hunk.hunk_id),
        delta_id = esc_pg(&hunk.delta_id),
        path_before = sql_nullable_text(hunk.path_before.as_deref()),
        path_after = sql_nullable_text(hunk.path_after.as_deref()),
        hunk_index = hunk.hunk_index,
        old_start = hunk.old_start,
        old_line_count = hunk.old_line_count,
        new_start = hunk.new_start,
        new_line_count = hunk.new_line_count,
        added_lines_json = sql_json_value_for_dialect(dialect, &added_json),
        deleted_lines_json = sql_json_value_for_dialect(dialect, &deleted_json),
        patch = esc_pg(&hunk.patch),
        now_sql = now_sql,
    ))
}
