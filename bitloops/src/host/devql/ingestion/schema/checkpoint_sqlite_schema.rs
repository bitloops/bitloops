...
ON checkpoint_files (repo_id, agent, event_time);

CREATE INDEX IF NOT EXISTS checkpoint_files_event_time_idx
ON checkpoint_files (repo_id, event_time);
...
pub(crate) fn checkpoint_runtime_schema_sql_sqlite() -> &'static str {
    CHECKPOINT_RUNTIME_SCHEMA_SQL_SQLITE
}

pub(crate) fn checkpoint_relational_schema_sql_sqlite() -> &'static str {
    CHECKPOINT_RELATIONAL_SCHEMA_SQL_SQLITE
}
