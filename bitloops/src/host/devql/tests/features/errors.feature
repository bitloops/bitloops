Feature: DevQL error BDD scenarios
  ...
  Scenario: ERR2 Invalid stage composition
    When devql parses the query:
      """
      repo("temp2")->artefacts(kind:"function")->dependencies(kind:"calls")->chatHistory()->limit(10)
      """
    And devql executes the query without a Postgres client
    Then the query fails with message containing "dependencies() cannot be combined with chatHistory()"
    And devql logs a validation-error event containing "dependencies() cannot be combined with chatHistory() stage"
