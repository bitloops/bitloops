...
"#;
    ...
    .await
    .expect("materialize cached path");
    crate::host::devql::sync::semantic_projector::project_path(
        &cfg,
        &relational,
        &desired,
        &extraction,
    ...
    .await
    .expect("project current semantic rows");
    crate::host::devql::sync::materializer::remove_path(&cfg, &relational, path)
        .await
        .expect("remove materialized path");
    crate::host::devql::sync::semantic_projector::remove_path(&cfg, &relational, path)
        .await
        .expect("remove semantic projection path");
    ...
    assert_eq!(semantics_count, 0);
    assert_eq!(features_count, 0);
...
#[tokio::test]
async fn full_sync_populates_current_semantic_rows_without_embeddings() {
    let repo = seed_full_sync_repo();
    let cfg = sync_test_cfg_for_repo(repo.path());
    let sqlite_path = repo.path().join("devql.sqlite");
    let relational = sqlite_relational_store_with_sync_schema(&sqlite_path).await;
    ...
    assert!(
        semantics_count > 0,
        "sync should project current semantic summaries"
    );
    assert!(
        features_count > 0,
        "sync should project current semantic features"
    );
    assert_eq!(
        embeddings_count, 0,
        "without an embedding provider, current embeddings stay empty"
    );
    assert_eq!(
        clone_edge_count, 0,
        "without embeddings, current clone edges should be cleared"
    );
}
