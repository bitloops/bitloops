...
pub(super) fn map_turn_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<InteractionTurn> {
    let has_token_usage: i64 = row.get(14)?;
    let files_modified_json: String = row.get(25)?;
    let files_modified =
    ...
    Ok(InteractionTurn {
        ...
        turn_number: u32::try_from(row.get::<_, i64>(8)?).unwrap_or_default(),
        prompt: row.get(9)?,
        ...
        summary: row.get(20)?,
        prompt_count: u32::try_from(row.get::<_, i64>(21)?).unwrap_or_default(),
