...
pub(crate) fn collect_rust_rstest_templates(
    ...
    while let Some(node) = stack.pop() {
        if node.kind() == "function_item" {
            ...
            {
                let seeds = build_rust_rstest_cases(name, node, source, &raw_attributes, 0, true);
                if !seeds.is_empty() {
...
pub(crate) fn build_rust_rstest_cases(
    ...
    let parameter_expansions =
        extract_rstest_parameter_expansions(function_node, source.as_bytes());
    if !parameter_expansions.is_empty() {
        if parameter_expansions
            .iter()
            .all(|expansion| expansion.statically_visible)
        {
