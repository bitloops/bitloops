...
//!   (`knowledge.add` vs `knowledge.refresh`, etc.). When runs go through [`DevqlCapabilityHost::invoke_ingester`](crate::host::devql::capability_host::DevqlCapabilityHost::invoke_ingester),
...
//!   under `engine::adapters::connectors`, selected via [`ConnectorRegistry`](crate::host::devql::capability_host::gateways::ConnectorRegistry) on the ingest context — not a separate pack-local provider module.
