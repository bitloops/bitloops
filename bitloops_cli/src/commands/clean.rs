...
fn run_clean_with_items_at_root(
    ...
    let total_deleted = deleted_states.len() + deleted_checkpoints.len();
    let total_failed = failed_states.len() + failed_checkpoints.len();
...
mod tests {
    ...
    fn TestRunClean_PreviewMode() {
        ...
        assert!(
            output.contains("No orphaned items"),
            "expected no-op cleanup output for shadow-only data, got: {output}"
        );
        assert!(
    ...
    fn TestRunClean_ForceMode() {
        ...
        assert!(
            branch_exists(&root, "bitloops/abc1234"),
            "shadow branch should be untouched in DB-only cleanup"
        );
        assert!(
            branch_exists(&root, "bitloops/def5678"),
            "shadow branch should be untouched in DB-only cleanup"
    ...
    fn TestRunCleanWithItems_PartialFailure() {
        let repo = setup_clean_test_repo();
        let root = repo.path().to_path_buf();
        create_orphan_session_state(&root, "orphan-session-ok");

        let mut stdout = Cursor::new(Vec::new());
        let items = vec![
            CleanupItem {
                item_type: CleanupType::SessionState,
                id: "orphan-session-ok".to_string(),
                reason: "test".to_string(),
            },
            CleanupItem {
                item_type: CleanupType::SessionState,
                id: "orphan-session-missing".to_string(),
                reason: "test".to_string(),
            },
        ];
    ...
    fn TestRunCleanWithItems_AllFailures() {
        ...
        let items = vec![
            CleanupItem {
                item_type: CleanupType::SessionState,
                id: "orphan-session-missing-1".to_string(),
                reason: "test".to_string(),
            },
            CleanupItem {
                item_type: CleanupType::SessionState,
                id: "orphan-session-missing-2".to_string(),
                reason: "test".to_string(),
            },
        ];
    ...
    fn TestRunCleanWithItems_MixedTypes_Preview() {
        ...
        assert!(
            output.contains("Found 2 orphaned items"),
