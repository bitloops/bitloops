...
pub fn check_knowledge_storage(ctx: &dyn CapabilityHealthContext) -> CapabilityHealthResult {
    ...
    if let Err(err) = stores.check_relational() {
        return CapabilityHealthResult::failed("knowledge relational store unavailable", err.to_string());
    }
    if let Err(err) = stores.check_documents() {
        return CapabilityHealthResult::failed("knowledge documents store unavailable", err.to_string());
    }
