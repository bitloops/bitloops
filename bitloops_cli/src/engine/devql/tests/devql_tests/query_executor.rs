...
#[tokio::test]
async fn execute_devql_query_rejects_semantic_neighbors_without_artefacts() {
    let cfg = executor_test_cfg();
    let parsed =
        parse_devql_query("repo(\"repo-1\")->semanticNeighbors()").expect("parsed devql query");
    let err = execute_devql_query(
        &cfg,
        &parsed,
    ...
    .await
    .expect_err("semantic neighbors without artefacts must fail");
...
#[tokio::test]
async fn execute_relational_pipeline_rejects_semantic_neighbors_for_sqlite() {
    let cfg = executor_test_cfg();
    let parsed = parse_devql_query("repo(\"repo-1\")->artefacts()->semanticNeighbors()")
        .expect("parsed devql query");
    ...
    let err = execute_relational_pipeline(
        &cfg,
        &executor_events_cfg(EventsProvider::DuckDb),
        &parsed,
        &relational,
    ...
    .await
    .expect_err("sqlite semantic neighbors must fail");

    assert!(
        err.to_string().contains("requires Postgres relational storage"),
        "unexpected error: {err}"
    );
}

#[tokio::test]
async fn execute_relational_semantic_neighbors_pipeline_rejects_sqlite() {
    let cfg = executor_test_cfg();
    let parsed = parse_devql_query("repo(\"repo-1\")->artefacts()->semanticNeighbors()->limit(3)")
        .expect("parsed devql query");
    let relational = RelationalStorage::Sqlite {
        path: tempfile::tempdir()
            .expect("temp dir")
    ...
    let err = execute_relational_semantic_neighbors_pipeline(
        &cfg,
        &executor_events_cfg(EventsProvider::DuckDb),
        &parsed,
        &relational,
