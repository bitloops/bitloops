...
mod tests {
    ...
    impl IngesterHandler for NoopIngesterHandler {
        fn ingest<'a>(
            &'a self,
            _request: crate::host::devql::capability_host::registrar::IngestRequest,
            _ctx: &'a mut dyn CapabilityIngestContext,
        ) -> BoxFuture<'a, Result<crate::host::devql::capability_host::registrar::IngestResult>>
        {
            Box::pin(async move {
                Ok(
                    crate::host::devql::capability_host::registrar::IngestResult::new(
                        json!({}),
                        "",
                    ),
                )
