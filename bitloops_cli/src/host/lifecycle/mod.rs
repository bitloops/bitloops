...
pub fn handle_lifecycle_session_start(
    ...
    let mut state = backend.load_session(&session_id)?.unwrap_or_else(|| {
        crate::host::session::state::SessionState {
...
pub fn handle_lifecycle_turn_start(
    ...
    let pre_prompt = crate::host::session::state::PrePromptState {
        session_id: session_id.clone(),
        ...
        ..crate::host::session::state::PrePromptState::default()
    };
    backend.save_pre_prompt(&pre_prompt)?;

    let registry = crate::host::strategy::registry::StrategyRegistry::builtin();
    let strategy = registry.get(
        crate::host::strategy::registry::STRATEGY_NAME_MANUAL_COMMIT,
        &repo_root,
    )?;
    ...
    let should_replace_bootstrap_prompt = state.step_count == 0
        ...
        && state.phase == crate::host::session::phase::SessionPhase::Idle;
...
pub fn handle_lifecycle_turn_end(
    ...
    let ctx = crate::host::strategy::StepContext {
...
pub fn capture_pre_prompt_state(
    ...
    use crate::host::session::state::PrePromptState as SessionPrePromptState;
