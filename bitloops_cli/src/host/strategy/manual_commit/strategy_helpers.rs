...
impl ManualCommitStrategy {
    ...
    fn condense_session(
        ...
        let transcript_content = if crate::host::session::legacy_local_backend_enabled() {
        ...
        let summary: Option<serde_json::Value> = {
            ...
            if settings.is_summarize_enabled() && !transcript_content.is_empty() {
                let summarize_agent = match state.agent_type.as_str() {
                    s if s == AGENT_TYPE_GEMINI => crate::host::summarize::AgentType::Gemini,
                    s if s == AGENT_TYPE_OPEN_CODE => crate::host::summarize::AgentType::OpenCode,
                    s if s == AGENT_TYPE_CLAUDE_CODE || s == AGENT_TYPE_CODEX => {
                        crate::host::summarize::AgentType::ClaudeCode
                    }
                    _ => crate::host::summarize::AgentType::Unknown,
                };
                ...
                if !scoped.is_empty() {
                    match crate::host::summarize::generate_from_transcript(
                        &scoped,
                        &state.files_touched,
                        summarize_agent,
                        None,
