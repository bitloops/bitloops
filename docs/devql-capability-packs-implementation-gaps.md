...
**Update (2026-03-21, follow-up):** **Linkage and classification ingesters** use the same **`Option<Arc<Mutex<BitloopsTestHarnessRepository>>>`** pattern as coverage. **`test_harness.linkage`** runs **`ingest_tests::execute`** (test discovery + **`replace_test_discovery`**); **`bitloops testlens ingest-tests`** invokes that ingester. **`test_harness.classification`** calls **`rebuild_classifications_from_coverage`**.

**Update (2026-03-23, CLI-1454):** Removed **`test_harness.summaries`** (read-only ingester). Commit-level row counts + **`coverage_exists_for_commit`** live on the **`test_harness_tests_summary`** stage (implementation in **`stages/tests_summary.rs`**) using **`query_context.resolved_commit_sha`**.
...
**Residual:** Optional **`TestHarnessIngestContext`** supertrait if you want compile-time separation from the shared mutex handle.
...
| 3     | Test Harness depth        | **Done:** coverage + linkage + classification ingesters + `testlens ingest-tests` / coverage via `invoke_ingester`; commit snapshot on **`test_harness_tests_summary`** stage (CLI-1454)                                                                 |
