...
`bitloops configure --web` owns daemon-level inference settings. `bitloops enable --install-embeddings` and `bitloops daemon enable --install-embeddings` can create the default local embeddings profile for you. `bitloops inference install` installs or repairs the managed standalone inference runtime, and interactive `bitloops enable` can attach summaries to Ollama automatically when it is available. Edit the daemon config manually only when you want a hosted profile or a customised local profile.
