...
const FASTEMBED_THREADS_ENV: &str = "FASTEMBED_THREADS";

pub fn get_cache_dir() -> String {
    std::env::var("FASTEMBED_CACHE_DIR").unwrap_or(DEFAULT_CACHE_DIR.into())
}

pub(crate) fn onnx_runtime_threads() -> usize {
    match std::env::var(FASTEMBED_THREADS_ENV) {
        Ok(value) => value
            .trim()
            .parse::<usize>()
            .ok()
...
mod tests {
    ...
    #[test]
    fn onnx_runtime_threads_respects_override() {
        let _guard = ENV_MUTEX.lock().expect("env mutex");
        let previous = std::env::var(FASTEMBED_THREADS_ENV).ok();
        unsafe {
            std::env::set_var(FASTEMBED_THREADS_ENV, "2");
        }

        assert_eq!(onnx_runtime_threads(), 2);

        match previous {
            Some(value) => unsafe { std::env::set_var(FASTEMBED_THREADS_ENV, value) },
            None => unsafe { std::env::remove_var(FASTEMBED_THREADS_ENV) },
        }
    }

    #[test]
    fn onnx_runtime_threads_falls_back_to_one_for_invalid_override() {
        let _guard = ENV_MUTEX.lock().expect("env mutex");
        let previous = std::env::var(FASTEMBED_THREADS_ENV).ok();
        unsafe {
            std::env::set_var(FASTEMBED_THREADS_ENV, "0");
        }

        assert_eq!(onnx_runtime_threads(), 1);
